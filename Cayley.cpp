 ...
 */
Cayley_Model::Cayley_Model(int problem_size, int sel_size, double * lower_theta_bound, double * upper_theta_bound)
{
 ...
 */
bool Cayley_Model::Learn(int ** samples, int size, double * weights, int * chosen){
 ...
 */
int Cayley_Model::Sample(CPopulation * population, int num_samples, int inverse, PBP * problem)
{
    int i, samples;
    for (samples=0;samples<num_samples ;samples++){
        ...
        if (population->Exists(m_sampling_permutation, m_problem_size, m_problem_size)==false){
        if (inverse)
 ...
 */
double Cayley_Model::CalculateDistancetoSample(int * solution, int ** samples, int cases_num){
 ...
 * Calculates the sum of Cayley distances of the given solution to the sample of solutions.
 */
double Cayley_Model::CalculateDistancetoSample_WeightedSamples(int * solution, int ** samples, int cases_num, double * weights){
    
    double dist=0;
    for (int i=0;i<cases_num;i++){
        Invert(samples[i],m_problem_size,m_inverted);
        Compose(solution, m_inverted, m_composed, m_problem_size);
        dist+=(double)CalculateDistanceAndX(m_composed, NULL)*(weights[i]);
    }
    return dist;
}
 ...
 * Calculates de consensus permutation from the given population cases.
 */
void Cayley_Model::CalculateConsensusRanking_WeightedSamples(int** samples, int sample_size, double * weights, int* consensus_ranking)
{
 ...
 * Calculates de consensus permutation from the given population cases.
 */
void Cayley_Model::CalculateConsensusRanking_WeightedSamples(int** samples, int sample_size, double * weights, int* consensus_ranking, int * chosen)
{
    //Option 1. Choose the solution that minimizes the distance to the sample.
    int best_index=-1;
    double best_distance=MAX_INTEGER;
 ...
 */
double f_Cayley(double theta, void * d) {
 ...
 */
double fdev_Cayley(double theta, void * d) {
        cayley_data * data = (cayley_data*)d;
    double sum=0;
    for(int j = 1 ; j < data->n ; j++)
 ...
 */
double Cayley_Model::CalculateThetaParameter(int*consensus, int ** samples, int cases_num)
{
 ...
 * Calculates the spread theta parameter from the m_consensus_ranking and the individuals in the population.
 */
double Cayley_Model::CalculateThetaParameter_WeightedSamples(int*consensus_ranking, int** samples, int samples_num, double * weights)
{
    //Option 2. Calculate the theta parameter according to the spread of the solutions around the consensus ranking.
    int s;
    double distance= 0;
    ...
    for(s = 0 ; s < samples_num ; s ++){
        Compose(samples[s], m_inverted, m_composed, m_problem_size);
        distance += (double)CalculateDistanceAndX(m_composed, NULL)*(weights[s]);
    }
 ...
 */
double Cayley_Model::CalculatePsiConstants(double theta, double* psi)
{
    int i,j;
    for (i = 0 ; i <  m_problem_size - 1; i ++ ){
        j=i+1;
...
double Cayley_Model::Probability(int * individual){
   ...
   // if (probability==0){ cout<<"¡¡Error numérico en Cayley-Probability!!"<<endl;exit(1);}
    return probability;
...
void Cayley_Model::GeneratePermuFromX(int *x, int *sigma){
    ...
    if (CAYLEY_SAMPLING==1)
    {
        ...
        int random, aux,i;
        for (  i = 0 ; i < m_problem_size ; i++) sigma[ i ] = i ;
        for (  i = 0 ; i < m_problem_size - 1 ; i++)
        {
            if ( x [ i ] == 1 ){
...
{
    //also updates the x vector if it is not null
...
int Cayley_Model::GetMostProbCycle(int ind, int **cycles, int len, int *leng_cycles){
    ...
    for(i=cont ; i < len; i ++ ){
        for(j=cont;j<len;j++){
            //if(j==i)proba_cycles[ i ] *= (double)gen.factorial(lengCycles[ j ]);
            //else proba_cycles[ i ] *= (double) gen.factorial(lengCycles[ j ]-1);
 ...
 */
double Cayley_Model::NormalizationConstant()
{   double value=1;
    for (int i=0;i<m_problem_size-1;i++)
 ...
 */
double Cayley_Model::Exponent(int * solution){
